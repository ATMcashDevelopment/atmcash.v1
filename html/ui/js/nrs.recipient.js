...
var NRS = (function(NRS, $, undefined) {
	NRS.automaticallyCheckRecipient = function() {
		...
		$recipientFields.on("checkRecipient", function() {
			...
			if (value && value != "ATM-____-____-____-_____") {
	...
	$("#send_money_amount").on("input", function(e) {
		...
		$(this).closest(".modal").find(".advanced_fee").html(NRS.formatAmount(NRS.convertToNQT(fee)) + " ATM");
	});
			...
			if (response.publicKey) {
				...
				if (response.errorCode) {
					if (response.errorCode == 4) {
						callback({
							"type": "danger",
							"message": $.t("recipient_malformed") + (!/^(ATM\-)/i.test(accountId) ? " " + $.t("recipient_alias_suggestion") : ""),
							"account": null
						});
	...
	NRS.checkRecipient = function(account, modal) {
		...
		if (/^(ATM\-)?[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(account)) {
