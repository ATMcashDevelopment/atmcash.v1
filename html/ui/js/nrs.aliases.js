...
var NRS = (function(NRS, $, undefined) {
		...
		if (alias) {
			...
			NRS.sendRequest("getAlias", {
				...
				if (response.errorCode) {
					...
					} else if ((aliasURI = /acct:(.*)@ATM/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
	...
	NRS.forms.setAlias = function($modal) {
		...
		if (data.type == "account") {
			if (!(/acct:(.*)@ATM/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
				if (/^(ATM\-)/i.test(data.aliasURI)) {
					...
					if (!address.set(data.aliasURI)) {
						...
						data.aliasURI = "acct:" + data.aliasURI + "@ATM";
					}
	...
	function setAliasType(type, uri) {
		...
		if (type == "uri") {
			...
			$("#register_alias_uri").val("").mask("ATM-****-****-****-*****");

			if (uri) {
				var match = uri.match(/acct:(.*)@ATM/i);
				if (!match) {
				...
				} else if (!/^ATM\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(uri)) {
					uri = NRS.accountRS;
				}
